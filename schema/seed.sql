...
FROM
  users
WHERE
  github_username IN ('alice', 'bob', 'carol', 'dave');
...
FROM
  users
WHERE
  github_username IN ('eve', 'frank', 'grace');
...
FROM
  users
WHERE
  github_username IN ('judy', 'mallory', 'ned');
...
FROM
  users
WHERE
  github_username IN ('oscar', 'peggy');
...
FROM
  users
WHERE
  github_username IN ('victor', 'walter', 'xavier', 'yvonne', 'zara');
  ...
  (
    ...
    (
      ...
      FROM
        users
      WHERE
        github_username = 'peggy'
    ),
    '2025-02-15 13:00:00+00',
    true,
    1,
    'FLAG{osint2}'
  ),
