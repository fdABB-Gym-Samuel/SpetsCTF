<script lang="ts">
	...
	let originalClasses = $derived(
		classesScoreboard.map((elem) => elem.className),
	);
	...
	let filteredUsers = $derived.by(() => {
		if (includeClasses.length >= 1) {
			return usersScoreboard
				.filter(
					(u) =>
						u.represents_class && includeClasses.includes(u.represents_class),
				)
				.slice(0, 15);
		} else {
			return usersScoreboard;
				...
				{:else}
					...
					<table class="w-full table-fixed">
									...
									usersScoreboard
										.filter(
											(user) =>
												...
												includeClasses.includes(user.represents_class),
										)
										.slice(0, 15).length -
										1
										? ''
                        ...
                        {i === 0
								...
								>
									<td class="h-12 w-12 pl-4 text-left sm:w-18 sm:pl-10"
										>{i + 1}</td
									>
									<td
