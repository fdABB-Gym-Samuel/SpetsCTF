<script lang="ts">
    ...
    let challengeResources: Record<'command' | 'website', string[]> = $state({
        command: resourceData
            ?.filter(
                (res: { type: 'cmd' | 'web' | 'file'; content: string }) =>
                    res.type === 'cmd'
            )
            .map(
                (res: { type: 'cmd' | 'web' | 'file'; content: string }) => res.content
            ),
    ...
    let original_files: string[] = $state(
        resourceData
            ?.filter(
                (res: { type: resource_type; content: string }) => res.type === 'file'
            )
...
<form
    ...
    {#if currentResourceType === 'website'}
        <Input
            ...
            pattern={linkPattern.source}></Input>
    {:else if currentResourceType === 'command'}
        <Input
            ...
            bind:value={currentResourceContent}></Input>
    {:else}
        <Input
            label="File"
            type="file"
            ...
            bind:value={currentResourceFile}></Input>
    {/if}
    <div class="mt-4">
        <Button
            type="submit"
