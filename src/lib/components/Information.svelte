...
<span bind:this={iconEl} role="tooltip" onmouseenter={show} onmouseleave={hide} class="inline-flex">
    <InfoIcon class="cursor-pointer" size={size} />
</span>
...
{#if visible}
    <div
        ...
        class="bg-black text-white text-sm rounded px-2 py-1 w-max max-w-xs z-50 pointer-events-none"
    >
        {text}
    </div>
{/if}